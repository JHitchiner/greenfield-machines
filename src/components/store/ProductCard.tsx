    ...
    >
      <Card className="overflow-hidden h-full flex flex-col group border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all duration-300">
        ...
        <CardContent className="p-5 flex-grow">
          <h3 className="text-lg font-display font-bold mb-2 line-clamp-1 group-hover:text-amber-600 transition-colors">
            {machine.name}
          </h3>
          ...
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(machine.specs).slice(0, 2).map(([key, value]) => (
              <div key={key} className="bg-slate-50 dark:bg-slate-900 p-2 rounded border border-slate-100 dark:border-slate-800">
        ...
        <CardFooter className="p-5 pt-0 flex items-center justify-between mt-auto">
          ...
          <Button 
            ...
            size="sm" 
            className="rounded-full bg-slate-900 dark:bg-slate-100 dark:text-slate-900 hover:bg-amber-600 dark:hover:bg-amber-500 transition-colors"
          >
            Add to Quote
          </Button>
        </CardFooter>
